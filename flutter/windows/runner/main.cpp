...
typedef char** (*FUNC_remotend_CORE_MAIN)(int*);
typedef void (*FUNC_remotend_FREE_ARGS)( char**, int);
const char* uniLinksPrefix = "remotend://";
...
{
  HINSTANCE hInstance = LoadLibraryA("libremotend.dll");
  if (!hInstance)
  {
    std::cout << "Failed to load libremotend.dll." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_remotend_CORE_MAIN remotend_core_main =
      (FUNC_remotend_CORE_MAIN)GetProcAddress(hInstance, "remotend_core_main_args");
  if (!remotend_core_main)
  {
    std::cout << "Failed to get remotend_core_main." << std::endl;
    return EXIT_FAILURE;
  }
  FUNC_remotend_FREE_ARGS free_c_args =
      (FUNC_remotend_FREE_ARGS)GetProcAddress(hInstance, "free_c_args");
  if (!free_c_args)
  {
  ...
  int args_len = 0;
  char** c_args = remotend_core_main(&args_len);
  if (!c_args)
  {
    std::string args_str = "";
  ...
  // Uri links dispatch
  HWND hwnd = ::FindWindow(_T("FLUTTER_RUNNER_WIN32_WINDOW"), _T("remotend"));
  if (hwnd != NULL) {
