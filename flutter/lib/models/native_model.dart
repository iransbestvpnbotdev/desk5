...
class PlatformFFI {
  ...
  remotendImpl get ffiBind => _ffiBind;
  ...
  Future<void> init(String appType) async {
    ...
    final dylib = Platform.isAndroid
        ? DynamicLibrary.open('libremotend.so')
        : Platform.isLinux
            ? DynamicLibrary.open('libremotend.so')
            : Platform.isWindows
                ? DynamicLibrary.open('libremotend.dll')
                : Platform.isMacOS
                    ? DynamicLibrary.open("liblibremotend.dylib")
  ...
  void _startListenEvent(remotendImpl remotendImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = remotendImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
